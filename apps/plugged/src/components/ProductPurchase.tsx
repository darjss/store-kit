...
import { createProductPurchaseController } from '@store-kit/storefront/purchase'
...
export function ProductPurchase(props: { product: ProductPurchaseData }) {
  const purchase = createProductPurchaseController(props.product)
  ...
  const chooseVariant = (id: string) => {
    if (purchase.selectVariant(id)) animateImageStage()
  }

  const announcementText = () => {
    const announcement = purchase.announcement()
    if (announcement?.type === 'quantity-clamped')
      return `Энэ сонголтоос ${announcement.maximum} ширхэг үлдсэн байна.`
    return announcement?.type === 'added' ? `${props.product.name} сагсанд нэмэгдлээ.` : ''
  }
  ...
  return (
    <section class="contents" aria-label="Барааны сонголт">
      ...
      >
        <Show when={purchase.selectedImage()}>
          {item => (
            <ProductImage
      ...
      <fieldset class="bg-paper m-0 border-0 p-4">
        ...
        <RadioGroup value={purchase.selectedVariantId()} onChange={chooseVariant}>
          <For each={props.product.variants}>
            {variant => (
              <label
                ...
                classList={{
                  'bg-acid': variant.id === purchase.selectedVariantId(),
      ...
      <div class="border-ink bg-paper-clean sticky bottom-0 z-10 flex min-w-0 gap-3 border-t-4 p-4 max-[340px]:flex-col max-md:bottom-[calc(68px+env(safe-area-inset-bottom))] max-md:flex-wrap max-md:p-2">
        <ButtonGroup class="max-[340px]:self-start" aria-label="Тоо ширхэг">
          <Button
            ...
            type="button"
            variant="outline"
            onClick={purchase.decrementQuantity}
          ...
          <Button
            ...
            type="button"
            variant="outline"
            disabled={purchase.quantity() >= purchase.maximumQuantity()}
            onClick={purchase.incrementQuantity}
            aria-label={`Нэгээр нэмэх. Дээд хэмжээ ${purchase.maximumQuantity()}`}
          >
            +
          </Button>
        </ButtonGroup>
        <Button
          ...
          type="button"
          disabled={!purchase.selectedVariant() || purchase.selectedVariant()?.stockQuantity === 0}
          onClick={purchase.addToCart}
        >
          Сагсанд нэмэх ·{' '}
          {formatMnt((purchase.selectedVariant()?.priceMnt ?? 0) * purchase.quantity())}
        </Button>
      </div>
