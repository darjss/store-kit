...
// The template deployment uses one R2 custom domain for its media. A
// store-level production origin is provisioned per customer later.
export const templateMediaBaseUrl = 'https://template-store.storekitcdn.darjs.dev/'
...
export type TemplateRuntimeEnvironment = {
...
export const validateTemplateEnvironment = (
  runtimeEnv: TemplateRuntimeEnvironment,
  ...
  const environment = createEnv({
    ...
    onValidationError: issues => {
      ...
      throw new Error(`Invalid Template Store environment. ${details}`)
    },
  })
  ...
  if (mediaBaseUrl !== templateMediaBaseUrl) {
    throw new Error(
      `PUBLIC_MEDIA_BASE_URL must use the template media domain. Expected: ${templateMediaBaseUrl}`,
    )
  }
