...
const paymentSummary = (order: PublicOrder) =>
  order.payment
    ? `${order.payment.method === 'qpay' ? 'QPay' : 'Дансаар шилжүүлэх'} · ${paymentStatusLabel(order.payment.status)}`
    : 'Төлөв тодорхойгүй'
...
function OrderAction(props: {
  ...
  return (
    <Button
      type="button"
      class="text-on-accent hover:bg-accent-strong rounded-action mr-2 mb-2 min-h-11 bg-accent font-bold"
...
function StatusContent() {
  ...
  return (
    <Switch>
      <Match
        when={
          status.state()._tag === 'MissingToken' || status.state()._tag === 'InvalidStatusToken'
        }
      >
        <div class="grid min-h-[60vh] place-content-center text-center">
          <h1 class="text-3xl font-extrabold">Захиалга олдсонгүй.</h1>
          <p class="text-muted">Хувийн холбоосоо шалгана уу.</p>
          <a href="/">Нүүр рүү буцах</a>
        </div>
      </Match>
      <Match when={status.state()._tag === 'Hydrating' || status.state()._tag === 'Loading'}>
        <div class="grid min-h-[60vh] place-content-center text-muted">Захиалгыг шалгаж байна…</div>
      </Match>
      <Match when={status.state()._tag === 'TransportError'}>
        <div class="border-line bg-panel rounded-action mx-auto grid min-h-80 max-w-xl place-content-center border p-6 text-center">
          <h1 class="text-2xl font-extrabold">Төлөв татаж чадсангүй.</h1>
          <p class="text-muted">
            Хувийн холбоос хадгалагдсан. Сүлжээгээ шалгаад дахин оролдоно уу.
          </p>
          <OrderAction
      ...
      <Match when={privateOrder()}>
        {order => (
          <article class="mx-auto w-full max-w-4xl">
            <header class="border-line bg-panel rounded-action border p-6 md:p-8">
              <p class="m-0 text-sm font-bold text-muted">Захиалгын төлөв</p>
              <h1 class="font-number my-3 text-3xl font-extrabold">{order().number}</h1>
              <strong>{orderStatusLabel(order().status)}</strong>
              <ol class="mt-7 grid gap-2 md:grid-cols-5" aria-label="Захиалгын үе шат">
                <For each={orderStages}>
                  {(label, index) => (
                    <li
                      class="border-line border-t pt-2 text-sm font-bold"
                      classList={{
                        'text-ink': index() <= orderStageIndex(order().status),
                        'text-muted': index() > orderStageIndex(order().status),
                      }}
                      aria-current={
            ...
            <section
              class="border-line bg-panel rounded-action mt-6 border p-5"
              aria-busy={status.isClaimingBankTransfer() || status.isRefreshingQPay()}
            >
              <h2 class="m-0 text-xl font-extrabold">Төлбөр</h2>
              <p class="text-muted">{paymentSummary(order())}</p>
              <Show when={claimMessage() ?? refreshMessage()}>
                {message => (
                  <div
                    class="border-line bg-surface rounded-action my-4 border-l-4 border-l-accent p-4"
                    role="status"
                  >
                    {message()}
                  </div>
              ...
              <Show when={order().payment?.status === 'claimed'}>
                <p class="border-line bg-surface rounded-action inline-block border px-3 py-2 font-bold">
                  Баталгаажуулалт хүлээж байна
                </p>
              </Show>
            </section>
            <section class="border-line bg-panel rounded-action mt-6 border p-5">
              <h2 class="m-0 text-xl font-extrabold">Бараа</h2>
              <For each={order().lines}>
                {line => (
                  <div class="border-line flex justify-between gap-4 border-b py-4">
                    <div>
              ...
              <div class="flex justify-between pt-4 text-lg font-bold">
                <span>Нийт</span>
                <strong class="font-number">{formatMnt(order().totalMnt)}</strong>
              </div>
            </section>
            <section class="border-line bg-panel rounded-action mt-6 border p-5">
              <h2 class="m-0 text-xl font-extrabold">Хүргэлт</h2>
              <p>
                {order().district}, {order().khoroo}-р хороо
              </p>
              <p>{order().address}</p>
              <small class="text-muted">Төлбөр батлагдсаны дараа захиалгыг бэлтгэнэ.</small>
            </section>
          </article>
        )}
      </Match>
...
export function OrderStatusView(props: { orderId: string }) {
      ...
      {_mounted => {
        ...
        return (
          <QueryClientProvider client={client}>
            <HeadlessOrderStatus.Root orderId={props.orderId} store="template-store">
              <StatusContent />
