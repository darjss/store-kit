...
const cartItem: PersistedCartItem = {
  ...
  quantity: 2,
  productSlug: 'simple-shirt',
  productName: 'Энгийн цамц',
  variantName: 'Хар / M',
  options: { өнгө: 'Хар', хэмжээ: 'M' },
  image: {
    url: 'https://media.example.com/products/simple-shirt/black.webp',
    width: 1365,
    height: 1278,
    alt: 'Энгийн цамц',
  },
...
test('checkout reveals only touched errors until submit, then summarizes and focuses all errors', async () => {
  const view = render(() => <CheckoutView />)
...
test('checkout controls edit values shaped like the checkout details schema', async () => {
  ...
  expect(view.getByText('Энгийн цамц × 2')).toBeTruthy()
})
