...
import { createId, defaultCheckoutSettingsId } from '../ids'
import {
...
const line = {
  ...
  variantName: 'Black / No microphone',
  sku: 'PLG-TRUTHEAR-GATE-BLACK-NOMIC',
  options: { color: 'Black', microphone: 'No' },
  imageR2Key: 'products/product_truthear/gate.webp',
  imageWidth: 1135,
  imageHeight: 453,
  imageAlt: 'TRUTHEAR Gate',
  unitPriceMnt: 10_000,
  quantity: 1,
  lineTotalMnt: 10_000,
} as const
...
test('shopping schemas accept integer MNT snapshots and controlled states', () => {
  ...
  expect(Value.Check(insertOrderSchema, { ...order, id: createId('payment') })).toBe(false)
})
...
test('checkout settings require the singleton cfg TypeID when an ID is supplied', () => {
  ...
  expect(Value.Check(insertCheckoutSettingsSchema, { ...settings, id: createId('order') })).toBe(
    false,
  )
})
