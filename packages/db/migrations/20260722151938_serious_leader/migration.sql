...
INSERT INTO `__new_product_variant_image`(`product_id`, `variant_id`, `image_id`) SELECT `product_variant`.`product_id`, `product_variant_image`.`variant_id`, `product_variant_image`.`image_id` FROM `product_variant_image` INNER JOIN `product_variant` ON `product_variant`.`id` = `product_variant_image`.`variant_id`;--> statement-breakpoint
