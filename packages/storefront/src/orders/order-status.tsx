...
export function OrderStatusRoot(
  ...
  const state = createMemo<OrderStatusState>(() => {
    if (token() === undefined) return { _tag: 'Hydrating' }
    if (!token()) return { _tag: 'MissingToken' }

    const result = status.data
    if (result)
      return result.match<OrderStatusState>({
        err: () => ({ _tag: 'InvalidStatusToken' }),
        ok: order => ({ _tag: 'Ready', order }),
      })
    return status.isError ? { _tag: 'TransportError' } : { _tag: 'Loading' }
  })
