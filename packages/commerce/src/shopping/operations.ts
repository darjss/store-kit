...
import { cartValidationInputSchema } from '@store-kit/db/schemas/shopping'
import type { PersistedCartItem } from '@store-kit/db/schemas/shopping'
...
const invalidCart = (input: unknown) => ({
  ...
  fields: Value.Errors(cartValidationInputSchema, input).map(error => ({
...
export const validateCart = async (input: PersistedCartItem[]) => {
  ...
  if (!Value.Check(cartValidationInputSchema, input)) {
  ...
  const currentVariants = await findCartVariants(
    input.map(({ variantId, quantity }) => ({ variantId, quantity })),
  )
